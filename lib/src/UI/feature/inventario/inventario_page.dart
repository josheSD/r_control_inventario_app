...
class _InventarioPageState extends State<InventarioPage> {
    ...
    return SafeArea(
        child: Container(
            child: SingleChildScrollView(
                child: Column(
      children: [_buildBody(context, inventarioProvider)],
    ))));
  }

  Widget _buildBody(BuildContext context, InventarioProvider inventarioProvider) {
    ...
    return Container(
        ...
        child: FutureBuilder(
            ...
            builder: (BuildContext context,
                AsyncSnapshot<ResponseInventario> snapshot) {
              if (snapshot.hasData) {
                if (snapshot.data!.status) {
                  List<Inventario> inventarios = snapshot.data!.data;

                  return _buildLista(inventarios);
  ...
  Widget _buildLista(List<Inventario> inventarios) {
